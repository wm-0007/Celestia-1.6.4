...
constexpr int CLASSES_VISIBLE_AS_POINT = ~(Body::Invisible      |
                                           Body::SurfaceFeature |
                                           Body::Component      |
                                           Body::Diffuse);

constexpr int CLASSES_SECONDARY_ILLUMINATOR = Body::Planet      |
                                              Body::Moon        |
                                              Body::MinorMoon   |
                                              Body::DwarfPlanet |
                                              Body::Asteroid    |
...
{
    return (classification & CLASSES_VISIBLE_AS_POINT) != 0;
}

bool
Body::isSecondaryIlluminator() const
{
    return (classification & CLASSES_SECONDARY_ILLUMINATOR) != 0;
}
